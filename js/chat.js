...
clintIo.emit("sayHi", "FROM FE TO BE")
...
clintIo.emit("sayHi", "FROM FE TO FrontEnd App", (res) => {
    console.log({ res })
})
...
function getUserData() {
    axios({
        ...
        if (user.profileImage) {
            imagePath = `${baseURL}/users/getfile/${user.profileImage}`  //getfile not upload
        }
...
function showUsersData(users = []) {
    ...
    for (let i = 0; i < users.length; i++) {
        ...
        if (users[i].profileImage) {
            imagePath = `${baseURL}/users/getfile/${users[i].profileImage}`  // getfile not upload
        }
        cartonna += `
